...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SMKMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SMKMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SMKMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("smk-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("SMKMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running SMKMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadSMKMiner() exception");
    } catch (...) {
        LogPrintf("ThreadSMKMiner() exception");
    }

    LogPrintf("ThreadSMKMiner exiting\n");
}
