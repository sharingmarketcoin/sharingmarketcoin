...
#if defined(NDEBUG)
#error "SMK cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "SMK Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if(nHeight <= 1 && nHeight > 0) {
		...
		nSubsidy = 5 * COIN;
	} else if (nHeight > 2000 && nHeight <= 130000) {
		nSubsidy = 12 * COIN;
	} else if (nHeight > 130000 && nHeight <= 800000) {
		nSubsidy = 10 * COIN;
