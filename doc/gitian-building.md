...
*Setup instructions for a gitian build of SMK using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SMK
...
Clone the git repositories for smk and gitian and then checkout the smk version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the smk repository
...
To build SMK (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the smk repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/smk/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/SMKCRYPTO/SMK
...
./bin/gbuild --commit smk=${COMMIT} --url smk=${URL} ../smk/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit smk=${COMMIT} --url smk=${URL} ../smk/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit smk=${COMMIT} --url smk=${URL} ../smk/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/smk-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/smk-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/smk-build.assert
```
...
[smk/gitian.sigs](https://github.com/smk-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
